...
class ValveRouteManager(object):
    ...
    def __init__(self, dp_id, logger, faucet_mac, arp_neighbor_timeout,
    ...
    def _neighbor_resolver_pkt(self, vid, faucet_mac, faucet_vip, ip_gw):
        pass
    ...
    def _add_resolved_route(self, dp_id, port_no, vlan, ip_gw, ip_dst,
                            eth_src, eth_dst, is_updated):
        ofmsgs = []
        if self.routers:
        ...
        if self.use_group_table:
            ...
            if self.dp_id == dp_id:
                inst = [
                    valve_of.apply_actions(self._nexthop_actions(eth_src, eth_dst, vlan)),
                    valve_of.goto_table(self.eth_dst_table)]
            else:
                inst = [valve_of.apply_actions([valve_of.output_port(port_no)])]
        ofmsgs.append(self.valve_flowmod(
            self.fib_table,
            in_match,
    ...
    def _update_nexthop_cache(self, dp_id, port_no, vlan, eth_src, ip_gw):
        now = time.time()
        nexthop = NextHop(dp_id, port_no, vlan.vid, eth_src, now)
        nexthop_cache = self._vlan_nexthop_cache(vlan)
    ...
    def _update_nexthop(self, dp_id, vlan, port, eth_src, resolved_ip_gw, broadcast=False):
        is_updated = False
        routes = self._vlan_routes(vlan)
        _, _, cached_eth_dst = self._cached_nexthop_eth_dst(vlan, resolved_ip_gw)
        ofmsgs = []
        ...
        if self.use_group_table:
            ofmsgs.extend(
                self._update_nexthop_group(
                    ...
                    vlan, port, self.faucet_mac, eth_src))
        ...
        for ip_gw in set(routes.values()):
            for faucet_vip in vlan.faucet_vips_by_ipv(self.IPV):
                if ip_gw in faucet_vip.network:
                    ip_gws.append((ip_gw, self.faucet_mac, faucet_vip))
        return ip_gws
    ...
    def _add_unresolved_nexthops(self, vlan, ip_gws):
        ...
        for ip_gw, _, _, in ip_gws:
            if self._vlan_nexthop_cache_entry(vlan, ip_gw) is None:
                self._update_nexthop_cache(None, None, vlan, None, ip_gw)
    ...
    def _vlan_unresolved_nexthops(self, vlan, ip_gws, now):
        ...
        for ip_gw, faucet_mac, faucet_vip in ip_gws:
            ...
            ip_gw_with_retry_time = (ip_gw, faucet_mac, faucet_vip, last_retry_time)
    ...
    def resolve_gateways(self, vlan, now):
        ...
        ofmsgs = []
        for ip_gw, faucet_mac, faucet_vip, last_retry_time in cycle_unresolved_nexthops:
            nexthop_cache_entry = self._vlan_nexthop_cache_entry(vlan, ip_gw)
            if (self._is_host_fib_route(vlan, ip_gw) and
                ...
                resolve_flows = self.resolve_gw_on_vlan(
                    vlan, faucet_mac, faucet_vip, ip_gw)
                if last_retry_time is None:
                    self.logger.info(
    ...
    def _cached_nexthop_eth_dst(self, vlan, ip_gw):
        ...
        if (nexthop_cache_entry is not None and
            ...
            return (
                    nexthop_cache_entry.dp_id,
                    nexthop_cache_entry.port_no,
                    nexthop_cache_entry.eth_src)
            ...
            if vlan.ip_in_vip_subnet(dst_ip) and not vlan.is_faucet_vip(dst_ip):
                ...
                for faucet_vip in vlan.faucet_vips_by_ipv(self.IPV):
                    if dst_ip in faucet_vip.network:
                        ...
                        resolve_flows = self.resolve_gw_on_vlan(
                            vlan, self.faucet_mac, faucet_vip, dst_ip)
                        ofmsgs.extend(resolve_flows)
                        self.logger.info(
    ...
    def add_route(self, vlan, ip_gw, ip_dst):
        ...
        routes = self._vlan_routes(vlan)
        routes[ip_dst] = ip_gw
        dp_id, port_no, cached_eth_dst = self._cached_nexthop_eth_dst(vlan, ip_gw)
